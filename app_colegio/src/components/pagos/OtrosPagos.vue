            ...
            <v-flex sm12 lg12 md12>
                ...
                >
                    <template v-slot:items="props">
                        <td class="text-xs-left">{{'Serie "'+ props.item.serie+'" - factura no.'+props.item.factura}}</td>
...
import factura from './comprobantes/Factura'
import ProgressBar from 'vue-simple-progress'
export default {
  name: "detalle_otros_pagos",
  components: {
    detallePago,
    factura,
    ProgressBar
  ...
  methods: {
      onEventPagos(inscripcion, pagos){
          let self = this
